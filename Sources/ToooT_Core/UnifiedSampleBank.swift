    ...
    public func overwriteRegion(offset: Int, data: [Float]) {
        ...
        data.withUnsafeBufferPointer { buffer in
            if let base = buffer.baseAddress {
                cblas_scopy(Int32(data.count), base, 1, samplePointer.advanced(by: offset), 1)
            }
        }
    }
        ...
        while reader.status == .reading {
            if let sampleBuffer = output.copyNextSampleBuffer(),
               let blockBuffer = CMSampleBufferGetDataBuffer(sampleBuffer) {
                ...
                let tempBuffer = UnsafeMutablePointer<Float>.allocate(capacity: length / 4)
                ...
                if currentOffset + frameCount <= totalSamples {
                    cblas_scopy(Int32(frameCount), tempBuffer, 1, samplePointer.advanced(by: currentOffset), 1)
